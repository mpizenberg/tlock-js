...
describe("integration", () => {
    it("should be able to encrypt and decrypt with the testnet client", async () => {
        ...
        const ciphertext = await timelockEncrypt(1, Buffer.from(message), client)
    ...
    it("should be able to encrypt and decrypt with g1/g2 swapped non-rfc client", async () => {
        const client = nonRFCMainnetClient()
        const message = "hello world and other things"
        const ciphertext = await timelockEncrypt(1, Buffer.from(message), client)
        const plaintext = await timelockDecrypt(ciphertext, client)
